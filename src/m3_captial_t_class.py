...
class CapitalT(object):
    ...
    def __init__(self, intersection_center, width, height, letter_thickness):
        ...
        # --------------------------------------------------------------
        hx = intersection_center.x - (.5*width)
        hy = intersection_center.y
        vx = intersection_center.x - (.5*letter_thickness)
        vy = intersection_center.y
        self.h_rect = rg.Rectangle(rg.Point(hx, hy), rg.Point(hx + width, hy + letter_thickness))
        self.v_rect = rg.Rectangle(rg.Point(vx, vy), rg.Point(vx + letter_thickness, vy + height))
    ...
    def clone(self):
        ...
        #     clone.pdf.
        # --------------------------------------------------------------
        intersection_x = self.h_rect.corner_1.x + (.5*(self.h_rect.corner_2.x - self.h_rect.corner_1.x))
        intersection_y = self.h_rect.corner_1.y
        intersection = rg.Point(intersection_x, intersection_y)
        width = self.h_rect.corner_2.x - self.h_rect.corner_1.x
        height = self.v_rect.corner_2.y - self.v_rect.corner_1.y
        thick = self.h_rect.corner_2.y - self.h_rect.corner_1.y
        clone = CapitalT(intersection, width, height, thick)
